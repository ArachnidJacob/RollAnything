...
namespace SpiderWeb
{
    ...
    public class Chance
    {
        ...
        public static bool Roll(float odds)
        {
            ...
            if (dice <= odds)
            {
                //Debug.Log("TableRoll successful!");
                return true;
            }

            //Debug.Log("TableRoll failed. :(");
            return false;
        }
    ...
    public class KeyframeUtil
    {
        ...
        public static void SetKeyTangentMode(Keyframe keyframe, int leftRight, TangentMode mode)
        {
            ...
            if (GetKeyTangentMode(tangentMode, leftRight) == mode)
                return;
            Debug.Log("bug"); //Great, good to know
        }
    ...
    public static class CurveExtension
    {
        ...
        public static void UpdateTangentsFromMode(AnimationCurve curve, int index)
        {
            ...
            if (KeyframeUtil.GetKeyTangentMode(key, 0) != TangentMode.Smooth &&
                KeyframeUtil.GetKeyTangentMode(key, 1) != TangentMode.Smooth)
                return;
    ...
    public static class Calc
    {
        ...
        public static LayerMask GunsafeLayer()
        {
            ...
            LayerMask returnLayer = IncludeLayer(new string[]
                {"Default", "BlockSonar", "IgnoreSonar", "Torpedo", "Terrain", "IKCreature"});
            return returnLayer;
        }
        ...
        public static bool SpheresIntersect(float threshHold = 0.1f, Vector3 sp1pos = new Vector3(), float sp1rad = 1,
            Vector3 sp2pos = new Vector3(), float sp2rad = 1)
        {
        ...
        public static FixedJoint FindNearestTransform(List<FixedJoint> targetList, Transform caller)
        {
            ...
            //order by distance
            orderedList = targetList.OrderBy(go => Vector3.SqrMagnitude(caller.position - go.transform.position))
                .ToList();
        ...
        public static GameObject FindNearestGameObject(GameObject[] targetList, GameObject caller)
        {
            ...
            //order by distance
            orderedList = targetList
                .OrderBy(go => Vector3.SqrMagnitude(caller.transform.position - go.transform.position)).ToList();
        ...
        public static Rigidbody FindNearestRigidbody(List<Rigidbody> rbList, GameObject caller)
        {
            ...
            //order by distance
            orderedList = rbList.OrderBy(go => Vector3.SqrMagnitude(caller.transform.position - go.transform.position))
                .ToList();
    ...
    public class Cam
    {
        ...
        public static bool CanIsee(Camera cam, Vector3 v)
        {
            ...
            if (!Calc.WithinDistance(farClipPlane, cam.transform.position, v))
                return false; //if the vector is not within the max possible distance, return false
            ...
            if (pointOnCam.x < 0 || pointOnCam.x >
                                 1 //if its outside any of the pointoncam normalized values, return false
