...
public class TableEditor : Editor
{
    ...
    void OnEnable()
    {
        ...
        totalWeightMethod = targetObj.GetType().GetMethod("TotWeight"); // Get the total m_Weight
        TotalWeight();
    ...
    void DrawTypeFilters()
    {
        ...
        EditorGUILayout.BeginVertical();
        {
            ...
            if (oldFilterCount != filterCount) //only recalculate the m_Weight if we changed the filter
            {
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        {
            if (GUILayout.Button("TableRoll"))
                rollMethod.Invoke(targetObj, null);
            if (GUILayout.Button("TableRoll 100 times"))
            {
                for (int i = 0; i < 100; i++)
                    rollMethod.Invoke(targetObj, null);
            }
        ...
        EditorGUI.BeginChangeCheck();
        {
            EditorGUILayout.LabelField(
                "Drag and drop Entry objects onto the TableRoll Table to add them. Enforces unique objects.");
            ...
            showRollElements =
                EditorGUILayout.PropertyField(newEntryTable,
                    new GUIContent("TableRoll Table")); // EditorGUILayout.Foldout(showRollElements, "TableRoll Table");
            GUI.Box(GUILayoutUtility.GetLastRect(), "");
    ...
    void DrawRollElements(List<TableEntry> tableToDraw)
    {
        ...
        {
            EditorGUI.BeginChangeCheck(); //Check for changes in array to recalculate the total m_Weight
            {
                EditorGUILayout.Space();

                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.LabelField("W:", GUILayout.MinWidth(elementIconLabel.x),
                        GUILayout.MaxWidth(elementIconLabel.y));
                    EditorGUILayout.LabelField("Base", GUILayout.MinWidth(weightField.x),
                        GUILayout.MaxWidth(weightField.y));
                    EditorGUILayout.LabelField("Table ", GUILayout.MinWidth(weightField.x - 10),
                        GUILayout.MaxWidth(weightField.y));
                    EditorGUILayout.LabelField("Chance", GUILayout.MinWidth(percentageField.x),
                        GUILayout.MaxWidth(percentageField.y));
                    EditorGUILayout.LabelField("Will Drop#", GUILayout.MinWidth(willDropField.x),
                        GUILayout.MaxWidth(willDropField.y));
                    EditorGUILayout.LabelField("Drop #", GUILayout.MinWidth(percentageField.x),
                        GUILayout.MaxWidth(percentageField.y));
                    EditorGUILayout.LabelField("Value", GUILayout.MinWidth(percentageField.x),
                        GUILayout.MaxWidth(percentageField.y));
                    EditorGUILayout.LabelField("Total Spent", GUILayout.MinWidth(percentageField.x),
                        GUILayout.MaxWidth(percentageField.y * 2));
                }
                EditorGUILayout.EndHorizontal();
            ...
            if (EditorGUI.EndChangeCheck()) //If something changed in our loot table, recalculate total m_Weight        
                TotalWeight();
    ...
    /// <summary>
    /// Draws a TableRoll Object with a link to its object, the m_Weight and the percentage chance of appearing in this loot table
    /// </summary>    
    void DrawRollElement(TableEntry rollObject)
    {
        if (rollObject == null) return;
        if (rollObject.entry == null)
        {
            entriesToRemove.Add(rollObject);
            return;
        }
        ...
        {
            GUILayout.Box(
                new GUIContent(EditorGUIUtility.ObjectContent(rollObject.entry, rollObject.entry.GetType()).image),
                GUILayout.MinWidth(elementIconLabel.x), GUILayout.MaxWidth(elementIconLabel.y),
                GUILayout.MinHeight(elementIconLabel.x));
            //EditorGUI.DrawRect(GUILayoutUtility.GetLastRect(),new Color(0.1f, 0.1f, 0.1f, 0.1f));
            EditorGUILayout.BeginVertical();
            {
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.ObjectField(rollObject.entry, rollObject.entry.GetType(), false,
                        GUILayout.MinWidth(rollObject.entry.name.Length * 5));
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    EditorGUILayout.LabelField(rollObject.entry.weight.ToString(), GUILayout.MinWidth(weightField.x),
                        GUILayout.MaxWidth(weightField.y));
                    rollObject.tableBonus = EditorGUILayout.IntField(rollObject.tableBonus,
                        GUILayout.MinWidth(weightField.x), GUILayout.MaxWidth(weightField.y));
                    EditorGUILayout.LabelField(percentage.ToString() + "%", GUILayout.MinWidth(percentageField.x),
                        GUILayout.MaxWidth(percentageField.y));
                    rollObject.guaranteeDrop = EditorGUILayout.IntField(rollObject.guaranteeDrop,
                        GUILayout.MinWidth(willDropField.x), GUILayout.MaxWidth(willDropField.y));
                    GUILayout.Space(10);
                    EditorGUILayout.LabelField(rollObject.dropTimes.ToString(), GUILayout.MinWidth(weightField.x),
                        GUILayout.MaxWidth(weightField.y));
                    GUILayout.Space(10);
                    EditorGUILayout.LabelField(rollObject.entry.Value().ToString(), GUILayout.MinWidth(weightField.x),
                        GUILayout.MaxWidth(weightField.y));
                    GUILayout.Space(10);
                    EditorGUILayout.LabelField(rollObject.totalResourcesSpent.ToString(),
                        GUILayout.MinWidth(willDropField.x), GUILayout.MaxWidth(willDropField.y));
                }
                EditorGUILayout.EndHorizontal();
            }
    ...
    void ResizeEntryTable()
    {
        ...
        for (int i = 0; i < entryArraySize.intValue; i++)
        {
            ...
            if (entryCopyObj.objectReferenceValue == null)
            {
                nullCount++;
                continue;
            }
    ...
    {
        if (lastRolled == null || property == null || property.entry == null ||
            property.entry != (Entry) lastRolled.objectReferenceValue)
